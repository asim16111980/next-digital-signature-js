import fs from "fs";
import { NextRequest, NextResponse } from "next/server";
import { getPrivateKey, signMessage } from "@/app/lib/crypto-server";
import { parseForm } from "../_formidable/route";

export const config = {
  api: {
    bodyParser: false,
  },
};

export async function POST(req: NextRequest) {
  try {
    const { fields, files } = await parseForm(req as any);

    let message = "";

    if (fields && fields.message) {
      message = String(fields.message);
    } else if (files && files.file) {
      // التعامل مع ملفات واحدة أو متعددة
      const fileObj = Array.isArray(files.file) ? files.file[0] : files.file;

      // @ts-ignore: تجاوز TS لأن filepath مش موجودة في type
      const filepath = fileObj.filepath || fileObj.path;
      if (!filepath)
        return NextResponse.json(
          { error: "file path missing" },
          { status: 400 }
        );

      const buf = fs.readFileSync(filepath);
      message = buf.toString("utf8");
    } else {
      return NextResponse.json(
        { error: "no message or file" },
        { status: 400 }
      );
    }

    const privateKey = getPrivateKey();
    console.log(privateKey);
    
    const signatureBuf = signMessage({ privateKeyPem: privateKey, message });
    const signatureBase64 = signatureBuf.toString("base64");

    return NextResponse.json({ signature: signatureBase64 }, { status: 200 });
  } catch (e: any) {
    console.error(e);
    return NextResponse.json({ error: e.message }, { status: 500 });
  }
}
